...
## Overview
This document describes the complete file upload functionality implemented for the SRM Approval System's clarification workflow. The implementation provides secure file upload, validation, and download capabilities specifically for clarification responses.
...
### File Upload Restrictions for Clarifications
- **PDF Only**: Clarification file uploads now restricted to PDF format only
- **Enhanced Security**: Stricter validation for clarification documents
- **Context-Aware Validation**: Different file type rules for regular vs clarification uploads

## Features Implemented

### 1. Secure File Upload API (`/api/upload`)
- **Endpoint**: `POST /api/upload`
- **Purpose**: Handle file uploads during clarification responses
- **Security**: File type validation, size limits, secure filename generation
- **Storage**: Files stored in `/public/uploads/clarifications/`
- **Context-Aware**: Supports `isClarification` parameter for PDF-only validation
...
- **Features**: 
  ...
  - Context-aware file type validation (PDF-only for clarifications)
...
- **Features**:
  ...
  - Separate validation rules for clarifications (PDF-only)
...
- **Documents**: PDF ONLY
- **Rationale**: Standardized format for official clarification documents
...
formData.append('isClarification', 'true'); // PDF-only validation
...
### 1. File Validation
- Context-aware validation (PDF-only for clarifications)
...
1. Test clarification uploads with PDF files (should work)
2. Test clarification uploads with DOC/XLS files (should fail)
...
This implementation provides enhanced workflow efficiency for leave requests while maintaining strict security standards for clarification document uploads.